...
// ── Discovery / public pages — shown in the top nav bar ──
const navItems = [
  { to: "/student",            key: "nav.home"           },
...
// ── Personal pages — shown ONLY inside the profile dropdown ──
const personalItems = [
  { to: "/student/profile",        icon: FiUser,       label: "My Profile",     tKey: "nav.myProfile" },
  { to: "/student/resume-builder", icon: FiFileText,   label: "Resume Builder"                        },
  { to: "/student/billing",        icon: FiCreditCard, label: "Billing"                               },
  { to: "/student/settings",       icon: FiSettings,   label: "Settings",       tKey: "nav.settings"  },
];
...
export default function StudentNavbar() {
  ...
  const storeRecentSearch = (type, value) => {
    ...
    const next = [
      { type, value: trimmed },
      ...recentSearches.filter((item) => !(item.type === type && item.value === trimmed)),
    ].slice(0, 6);
    setRecentSearches(next);
    try { window.localStorage.setItem(searchStorageKey, JSON.stringify(next)); } catch { /* ignore */ }
  };
  ...
  useEffect(() => {
    ...
    const loadCounts = async () => {
      try {
        ...
        if (!mounted) return;
        const convItems = Array.isArray(convRes?.data) ? convRes.data : [];
        setMessagesUnread(convItems.reduce((sum, c) => sum + (Number(c?.unread) || 0), 0));
        setNotificationsUnread(Number(notifRes?.data?.unreadCount || 0));
      } catch {
        if (!mounted) return;
        setMessagesUnread(0);
        setNotificationsUnread(0);
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="flex h-16 w-full items-center gap-3 px-4 sm:px-6 lg:px-8">
        ...
        {/* Desktop nav — discovery pages only */}
        <nav className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => (
            <NavLink
              key={item.to}
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 mt-2 w-52 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
                ...
                {/* Personal tabs: Profile · Resume Builder · Billing · Settings */}
                {personalItems.map(({ to, icon: Icon, label, tKey }) => (
                  <button
                    key={to}
                    type="button"
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-sm text-slate-500 hover:bg-slate-50"
                >
                  <FiExternalLink size={15} className="shrink-0" />
                  Exit
                </button>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {recentSearches.length ? (
                          ...
                          <p className="text-[13px] font-semibold text-slate-500">
                            No recent searches yet. Your next search will appear here.
                          </p>
                        )}
                      </div>
                    </div>
      ...
      {mobileOpen && (
        <div className="border-t border-slate-200 bg-white md:hidden">
          <div className="w-full space-y-1 px-4 py-3 sm:px-6">
            ...
            <div className="mt-2 border-t border-slate-200 pt-2">
              ...
              <button
                ...
                className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold text-slate-500 hover:bg-slate-50 transition"
              >
