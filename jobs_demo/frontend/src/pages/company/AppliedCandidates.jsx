              ...
              <tbody>
                {filtered.map((a) => (
                  <tr key={a.id} className="border-t border-slate-100 hover:bg-blue-50/40">
                    ...
                    <td className="py-3"><div className="flex flex-wrap gap-1">{(a.skills || []).slice(0, 2).map((s, index) => <span key={`${a.id}_${s}_${index}`} className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-slate-600">{s}</span>)}</div></td>
